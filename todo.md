...
- **Credentials model — DECIDED (2026-06): Bring Your Own Secrets.** Free users' quantum-backend secrets (IBM / IonQ / BlueQubit) live on their **local device (browser)** and are injected **per-request**, used transiently in-memory for that one job, and **never persisted** by the backend (not in the DB, Job record, or logs). Paid users use **Kanad-owned server-side credentials** (`KANAD_IBM_TOKEN` / `KANAD_BLUEQUBIT_TOKEN` / …). This re-honors the §0 "no secrets at rest in the cloud" spirit for the BYO tier, without an external node. **Implemented this session:** persisted credential columns dropped (generated migration `20260602_01_drop_user_provider_credentials`, not yet applied), per-request `credentials` payload + process-local `_transient_credentials` + `_scrub_credentials_from_config`, resolution order = per-request → Kanad-owned server creds → legacy `.env` → none. Resolves the §2.3/§2.4 question: device-auth + local OS-keyring vault are **not** carried forward (the node is archived).
...
> **STATUS (2026-06): hardening complete.** All in-process execution guards landed and verified — backend boots clean, `pytest` 7/7, `next build` exit 0. The only open item is the PhysicsVQE live curve, which is blocked externally on kanad-core. The live multi-iteration streaming run + 503/timeout behavioural checks are owed against a logged-in browser (deferred to the `/chrome` pass).
- [x] **Multi-process fan-out via Redis** `[!]` (audit: high). DONE — optional Redis pub/sub relay in `ConnectionManager` (`init_redis`/`_redis_relay_loop`/`_publish_redis`), `PUBLISH calc_updates:{id}` on `send_to_job`, lifespan-gated on `settings.redis_url`. **Pure no-op when `REDIS_URL` unset / `redis` absent** (local single-process unchanged). NOTE for prod: add `redis` to `requirements.txt` to enable multi-worker. `ConnectionManager.active_connections` and the in-memory `calculations_db` dict are **process-local**. With `WEB_CONCURRENCY > 1`, a browser whose WS lands on Worker-B never sees frames from a job running on Worker-A → 100% live-frame loss (HTTP polling masks it). Fix: a `RedisOptionalEventBus` — defaults to in-process `connection_manager.dispatch`; when `REDIS_URL` is set + `redis` installed, ALSO `PUBLISH calc_updates:{calc_id}` and run a lifespan `SUBSCRIBE`-and-relay task that fans received frames through `send_to_job`. Add optional `redis_url` to `api/config.py` (default `None` = current single-worker behaviour). Redis/Celery are NOT installed today and there is no `redis_url` in config — this is greenfield.
- [x] **Execution scaling beyond `BackgroundTasks`** `[!]` (audit: high). DONE — concurrency gate honoring `config.max_concurrent_jobs` (default 5) via a single-event-loop counter `_active_job_count`; submit returns **HTTP 503** with a plain message when full; slot released in the guarded runner's `finally`. Now applies to **every** job (the WS-dispatch exemption was removed with the legacy node).
- [x] **Per-job timeouts** `[!]` (audit: high). DONE — `_run_calculation_guarded` runs the sync solver via `asyncio.to_thread` under `asyncio.wait_for(config.job_timeout_seconds)`; on timeout sets the cancel flag, marks the Job **FAILED** with "exceeded the N-second limit", and releases the slot. Cooperates with the existing cancel path (won't clobber a timeout-FAILED record).
- [x] **Resource guards** (audit: high). DONE — `_check_resource_feasibility` rejects FCI/CASCI past `fci_max_qubits` (default 20) and does a soft `psutil` RAM check for statevector (`statevector_ram_safety_factor`, `min_free_ram_mb`); `auto` solver exempt; degrades gracefully if `psutil` missing (installed: 7.2.2).
- [x] **Durable mid-run job state** (audit: medium). DONE — reused the existing `Job.result.convergence_history` JSON field (no migration); throttled snapshots (≥10 pts or ≥2 s) mid-run + optional Redis hash mirror (`calc_history:{id}`, 1h TTL); rehydrate-on-reconnect on WS connect and HTTP GET via a `set_history_provider` hook. A hard crash loses at most the last ~2 s of points by design.
...
> **STATUS (2026-06): DONE.** Browser-direct execution is now the only path; all legacy node code deleted and `import api.main` + `pytest` 7/7 verified clean. (Changes staged in the working tree, not yet committed.)
- [x] `api/routes/compute_ws.py` — DELETED (WS gateway + `NODE_REGISTRY`). Also removed the `/compute_status` admin endpoint that read its `session_summary`.
- [x] `api/routes/_compute_dispatch.py` — DELETED; the WS-first dispatch branch removed from `submit_calculation` (all jobs now in-process under the §9.3 guards).
- [x] `api/routes/device_auth.py` — DELETED (RFC 8628 device routes).
- [x] `api/routes/compute.py` — DELETED (polling endpoints).
- [x] `api/protocol.py` — DELETED (compute↔app wire format).
- [x] `api/workers/` — DELETED entirely (`base_worker` / `vqe_worker` / `md_worker` / `reaction_worker` / `celery_app` / `__init__`). Legacy `broadcast_*` helpers removed from `ConnectionManager` (only the workers used them; `send_to_job` + the Redis seam + history hooks kept). Dead tests removed (`test_compute_ws.py`, `test_device_auth.py`, `test_protocol_fuzz.py`).
- [x] `User.kanad_compute_key` + plaintext credential columns — DROPPED from the model + lightweight migrations; per the BYO decision above. Generated Alembic migration `20260602_01` (not yet applied to the dev DB). Orphaned ORM models `device_code.py` / `compute.py (ComputeNode)` remain as a small follow-up (tables not dropped).
- [x] **This entire repo (`kanad-compute`):** ARCHIVED — `ARCHIVED.md` added + `⚠️ ARCHIVED (2026-06)` banner prepended to `README.md`. Python node + Rust workspace preserved in git history (Phase 3 native VQE/L-BFGS kept for possible future in-process Rust acceleration), NOT on the critical path. Repo + history intentionally NOT deleted.

> **Follow-ups (next session).** (1) Frontend still references the removed creds contract — `web/src/lib/auth-store.ts` (reads `has_*` flags, PUTs persisted creds), `settings-store.ts` (`kanad_compute_url`), `BackendConfig.tsx`, `BackendSelector.tsx` (offers a `kanad_compute` backend that now silently falls back to statevector). Switch these to the per-request BYO model. (2) Owed verification against a logged-in browser: a long VQE/SQD run showing the live per-iteration curve, an oversubmit → 503, and a hung job → FAILED-at-timeout. (3) Apply migration `20260602_01` + drop the orphaned `device_codes`/`compute_nodes` tables.
